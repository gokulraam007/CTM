...
# Default values from EON TOPCon 108 cells module
col_reset = st.sidebar.columns([1, 1])
with col_reset[0]:
    if st.button("Reset to EON TOPCon Defaults"):
        st.session_state.reset = True

# Cell Parameters
st.sidebar.subheader("1. Solar Cell Parameters")
cell_power = st.sidebar.number_input("Cell Power (Wp)", min_value=4.0, max_value=7.0, value=5.5, step=0.1, 
                                      help="EON TOPCon typical: 5.5 Wp")
cell_efficiency = st.sidebar.number_input("Cell Efficiency (%)", min_value=20.0, max_value=26.0, value=24.5, step=0.1,
                                          help="EON TOPCon: 24.5% (typical)")
num_cells = st.sidebar.number_input("Number of Cells", min_value=60, max_value=144, value=108, step=1,
                                    help="EON TOPCon: 108 cells")

# Module Geometry
st.sidebar.subheader("2. Module Geometry")
module_area = st.sidebar.number_input("Module Area (m²)", min_value=1.0, max_value=3.0, value=2.38, step=0.01,
                                      help="EON TOPCon: 2.38 m²")
cell_area = st.sidebar.number_input("Cell Area (cm²)", min_value=200.0, max_value=300.0, value=274.5, step=0.5,
                                    help="M10 cells: 274.5 cm²")

# Calculate geometric factors
total_cell_area = (num_cells * cell_area) / 10000
inactive_area_fraction = 1 - (total_cell_area / module_area)

# Optical Loss Parameters
st.sidebar.subheader("3. Optical Loss Parameters")
glass_transmission = st.sidebar.slider("Glass Transmission (%)", 88.0, 96.0, 91.5, 0.5,
                                       help="EON: 91.5% (standard)")
eva_transmission = st.sidebar.slider("EVA Transmission (%)", 94.0, 98.0, 96.5, 0.5,
                                     help="EON: 96.5% (UV-stable)")

# Resistive Loss Parameters
st.sidebar.subheader("4. Resistive Loss Parameters")
num_busbars = st.sidebar.selectbox("Number of Busbars", [3, 5, 9, 12, 16], index=2,
                                   help="EON: 9 busbars (MBB)")
...
cell_binning_tolerance = st.sidebar.slider("Cell Binning Tolerance (±%)", 0.0, 5.0, 2.0, 0.5,
                                           help="EON: ±2.0% (tight sorting)")
...
with col_viz1:
    ...
    categories = ["Cell
Power", "Geometric", "Glass", "EVA", "Ribbon
Shade", 
                  "Coupling
Gain", "Resistive", "Mismatch", "JB & Cable", "Module
Power"]
    ...
    for i, bar in enumerate(bars):
        ...
        if height > 0:
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_y() + height/2, 
                   f'{height:.0f}W', ha='center', va='center', fontsize=8, fontweight='bold', color='white')
...
with col_viz2:
    ...
    # Prepare pie data (exclude coupling gain which is negative)
...
def create_pdf_report(total_cell_power, module_power, module_efficiency, df_losses, 
                     geometric_loss, net_optical_loss, total_resistive_loss, mismatch_loss, jb_cable_loss):
    ...
    summary_text = f"""
    This report presents a detailed Cell-to-Module (CTM) loss analysis for EON TOPCon solar modules. 
    The analysis identifies and quantifies all loss mechanisms between the individual solar cell efficiency 
    and the final module efficiency. Total CTM loss: <b>{total_ctm_loss:.2f}%</b>, resulting in 
    module power of <b>{module_power:.1f} Wp</b> from cell power of <b>{total_cell_power:.0f} Wp</b>.
    """
    ...
    references = """
    <b>1. Haedrich, I., et al. (2014).</b> "Unified methodology for determining CTM ratios." 
    <i>Solar Energy Materials and Solar Cells</i>, 131, 14-23.<br/><br/>

    <b>2. Mittag, M., et al. (2019).</b> "Cell-to-Module (CTM) Analysis for Photovoltaic Modules." 
    <i>IEEE PVSC Conference</i>.<br/><br/>

    <b>3. Hanifi, H., et al. (2017).</b> "Investigation of cell-to-module (CTM) ratios of PV modules by analysis 
    of loss and gain mechanisms." <i>Photovoltaics International</i>.<br/><br/>

    <b>4. Fraunhofer ISE (2020).</b> SmartCalc.CTM - Cell to Module Analysis Software. 
    www.cell-to-module.com<br/><br/>

    <b>5. IEC 61215 (2021).</b> Terrestrial photovoltaic (PV) modules - Design qualification and type approval.<br/><br/>

    <b>6. Roy, J.N., et al. (2016).</b> "Comprehensive analysis and modeling of cell to module (CTM) conversion loss." 
    <i>Solar Energy</i>, 135, 618-628.
    """
...
with col_download1:
    if st.button("Generate PDF Report", key="generate_pdf", use_container_width=True):
        pdf_buffer = create_pdf_report(total_cell_power, module_power, module_efficiency, df_losses,
                                       geometric_loss, net_optical_loss, total_resistive_loss, 
                                       mismatch_loss, jb_cable_loss)
...
1. **Haedrich, I., et al. (2014).** "Unified methodology for determining CTM ratios." 
   *Solar Energy Materials and Solar Cells*, 131, 14-23.

2. **Mittag, M., et al. (2019).** "Cell-to-Module (CTM) Analysis for Photovoltaic Modules." 
   *IEEE PVSC Conference*.

3. **Hanifi, H., et al. (2017).** "Investigation of cell-to-module (CTM) ratios of PV modules by analysis 
   of loss and gain mechanisms." *Photovoltaics International*.

4. **Fraunhofer ISE (2020).** SmartCalc.CTM - Cell to Module Analysis Software. [www.cell-to-module.com](http://www.cell-to-module.com)

5. **IEC 61215 (2021).** Terrestrial photovoltaic (PV) modules - Design qualification and type approval.

6. **Roy, J.N., et al. (2016).** "Comprehensive analysis and modeling of cell to module (CTM) conversion loss." 
   *Solar Energy*, 135, 618-628.
